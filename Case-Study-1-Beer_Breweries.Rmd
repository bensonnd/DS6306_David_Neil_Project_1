...
alcoholic content. One thing to note in particular was how many beers hover around the .05 ABV. This could be due to state laws or consumers' taste for moderate alcohol content in their beer._
```{r,warning=FALSE,message=FALSE}
...
## Analysis #7 
_Relationship between IBU and ABV? We do see a positive,moderate correlation between ABV and IBU with r value of .57; the projeced IBU increases as ABV increases to some degree. It's estimated that about 32.55% of the variation of a beer's bitterness can be attributed to how much alcohol the beer has._
```{r,warning=FALSE,message=FALSE}
...
## Analysis #8
_Difference of IBU and ABV between IPAs, Ales, and other? When using a KNN model, we can project a beer's predicted style just by reviewing its IBU with approximately 71.91% accuracy. With regards to IBU alone, identifying the beer's style as IPA is much easier than Ales or Other._
```{r,warning=FALSE,message=FALSE}
