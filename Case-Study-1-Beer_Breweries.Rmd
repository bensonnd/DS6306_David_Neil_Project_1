...
_EDA inference_
```{r}
#elevation <- read.csv("C:/Users/David/Google Drive/Masters/Data Files/Mean_Elevation_per_State.csv")
elevation <- read.csv("C:/Git/DS6306_David_Neil_Project_1/DS6306_David_Neil_Project_1/Data Sources/Mean_Elevation_per_State.csv")
elevation <- elevation %>% plyr::rename(c("Mean.elevationList.by.state"="Mean_Elevation"))
elevation$Mean_Elevation <- gsub("feet", "", elevation$Mean_Elevation)
elevation$Mean_Elevation <- gsub(",", "", elevation$Mean_Elevation)
elevation$Mean_Elevation <- as.integer(elevation$Mean_Elevation)
head(elevation, 52)

elevation %>% ggplot(aes(x = Mean_Elevation)) + 
  geom_histogram() + 
  ggtitle("Distribution of Elevetation") +
  ylab("Count") + xlab("Mean Elevation")

elevation <- elevation %>% select(State_Abr, Mean_Elevation)

summary_df <- merge(x=summary_df, y=elevation, by.x="State", by.y="State_Abr", all.x=TRUE)

fakename <- summary_df %>% ggplot(aes(x=Mean_Elevation, y=m)) + geom_point() + geom_smooth(method="lm") +
  labs(title="Relationship between ABV and IBU",
       caption="Source: beer, brewery data")
fakename_hist <- ggMarginal(fakename, type="histogram", fill="slateblue")#, xparams=list(bins=10), yparams=list(bins=10))
fakename_hist
