...
```{r}
# beer_df_cleaned %>% ggplot(aes(x = State,fill=State)) + geom_histogram(stat="count") + scale_y_continuous() +  theme(legend.position = "none") + ggtitle("# of Breweries per State")

brewery_count <- beer_df_cleaned %>% group_by(State) %>% summarise(Count=n())
brewery_count %>% ggplot(aes(x=reorder(State, -Count), y=Count, fill=State)) + geom_bar(stat='identity') +
  theme(legend.position = "none") + xlab("State") + ggtitle("Descending # of Breweries per State") 
...
library(ggExtra)
ABV_IBU_rel <- beer_df_cleaned %>% group_by(State) %>% ggplot(aes(x=ABV, y=IBU, fill=)) + geom_point() + geom_smooth(method="lm") +
