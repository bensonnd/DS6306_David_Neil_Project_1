...
present in each state. After counting the number of unique breweries per state, we found that Colorado leads in having 47 breweries with three states ND, SD, WV and DC having only 1 each._
```{r,warning=FALSE,message=FALSE}
brew %>%
  group_by(State) %>%
...
We found that Colorado has the most alcoholic beers while Oregon had the most bitter. Both of which are in the top 5 for number of breweries and top 6 for breweries per capita._
```{r}
...
Here we found that most of the beers fell within a normal distribution with most beers generally being somewhat equal in its alcoholic content. One thing to note in particular was the right skew in the distribution since the default deifition of "Beer" under federal law is 
anything greater than 0.5% alcohol thus explaning the skewness._
...
## Analysis #7 
_After examining how IBU differs from ABV from varying perspectives, we wanted to examine more closely the relationship between IBU and ABV. 
This was done simply by plotting each variable without any transformations applied as interpretation was important. We found that after even after imputing IBU values
with the mean per beer style, there is still mild evidence supporting a linear relationship between the two (corr=0.539)._
```{r,warning=FALSE,message=FALSE}
...
p2 <- p1 + scale_fill_gradient(low = "white", high = "#CB454A") + labs(title = "Per Capita Breweries per 1M Drinking Age Adults") + theme_map() + labs(fill = "Percent", caption="Source: beer, brewery data")
p2

# number of breweries, beers per capita, and breweries per capita in CO and TX
summary_df_co_TX <- summary_df %>% filter(State_Abvr == 'CO' | State_Abvr == 'TX') %>% select(State, Population, Breweries, per_capita_breweries, Beers, per_capita_beers, Median_ABV, Max_ABV, Median_IBU, Max_IBU)
